...
def build_opt_lr(model, current_epoch=0):
    ...
    trainable_params += [{'params': model.updatenet.parameters(),
                          'lr': cfg.UPDATE.LAYERS_LR * cfg.TRAIN.BASE_LR}]
        ...
        if epoch != idx // num_per_epoch + start_epoch:
            ...
            if epoch == cfg.TRAIN.EPOCH:
                torch.save({'epoch': epoch,
                            'state_dict': model.state_dict(),
                            'optimizer': optimizer.state_dict()},
                           cfg.TRAIN.SNAPSHOT_DIR + '/checkpoint_efinal.pth')
                return
        ...
        if epoch != idx // num_per_epoch + start_epoch:
            ...
            if epoch == cfg.TRAIN.EPOCH:
                torch.save({'epoch': epoch,
                            'state_dict': model.module.state_dict(),
                            'optimizer': optimizer.state_dict()},
                           cfg.TRAIN.SNAPSHOT_DIR + '/checkpoint00_efinal.pth')
                return
...
def main():
    ...
    # create model
    if mode == 'single':
        model = ModelBuilder_tctrack('train').train().cuda()
    else:
