...
def ca():
    path='/media/hasil/Data_SSD/test_set/UAVTrack112_l'
    namefile=os.listdir(path+'/anno')
    namefile.sort()
    
    name_list=[]
    for seqname in namefile:
...
class UAVTrack112lDataset(Dataset):
    """
    Args:
    ...
    def __init__(self, name, dataset_root, load_img=False):
        super(UAVTrack112lDataset, self).__init__(name, dataset_root)
        meta_data = ca()
