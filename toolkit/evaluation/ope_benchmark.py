...
dataset_root = os.path.join(os.path.dirname(__file__), '../../tools/results/DTB70/snapshot_amst.txt')
...
class OPEBenchmark:
    ...
    def show_result(self, success_ret, precision_ret=None,
        ...
        with open(dataset_root, 'a') as f:
            tmp = formatter.format(tracker_name, success, norm_precision, precision)
            f.writelines(tmp + '\n')
        print('-'*len(header))
