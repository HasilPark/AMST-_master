...
class Tranformer_spatio_encoder(Module):

    def __init__(self, d_model: int = 512, nhead: int = 8, num_encoder_layers: int = 6, dim_feedforward: int = 384,
                 dropout: float = 0.1,
                 activation: str = "relu", custom_encoder: Optional[Any] = None) -> None:
        super(Tranformer_spatio_encoder, self).__init__()
...
class TransformerEncoderLayer(Module):
    ...
    def forward(self, src: Tensor, srcc: Tensor, src_mask: Optional[Tensor] = None,
        ...
        src = self.cross_attn(src.view(b, c, int(s ** 0.5), int(s ** 0.5)) \
                              , srcc.contiguous().view(b, c, int(s ** 0.5), int(s ** 0.5))).view(b, c, -1).permute(2, 0,
                                                                                                                   1)
