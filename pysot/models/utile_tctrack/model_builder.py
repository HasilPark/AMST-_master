...
class ModelBuilder_tctrack(nn.Module):
    ...
    def forward(self, data, videorange):
        ...
        for i in range(len(xf)):
            xf[i] = xf[i].view(cfg.TRAIN.BATCH_SIZE // cfg.TRAIN.NUM_GPU, videorange + 1, xf[i].size(-3),
                               xf[i].size(-2), xf[i].size(-1))

        loc_b, cls2_b, cls3_b = self.grader(xf, zf)
        loc_o, cls2_o, cls3_o = self.grader(xf, update_f)
        loc_u, cls2_u, cls3_u = self.grader(xf, uf)

        cls2_b = self.log_softmax(cls2_b)
        cls_loss2_b = select_cross_entropy_loss(cls2_b, labelcls2)
        cls_loss3_b = self.cls3loss(cls3_b, labelcls3)
        pre_bbox_b = self.getcentercuda(loc_b)

        cls2_o = self.log_softmax(cls2_o)
        cls_loss2_o = select_cross_entropy_loss(cls2_o, labelcls2)
        cls_loss3_o = self.cls3loss(cls3_o, labelcls3)
        pre_bbox_o = self.getcentercuda(loc_o)

        cls2_u = self.log_softmax(cls2_u)
        cls_loss2_u = select_cross_entropy_loss(cls2_u, labelcls2)
        cls_loss3_u = self.cls3loss(cls3_u, labelcls3)
        pre_bbox_u = self.getcentercuda(loc_u)

        bbo = self.getcentercuda(labelxff)

        loc_loss1_b, loc_loss3_b = self.IOULOSS(pre_bbox_b, bbo, weightxff)
        loc_loss1_o, loc_loss3_o = self.IOULOSS(pre_bbox_o, bbo, weightxff)
        loc_loss1_u, loc_loss3_u = self.IOULOSS(pre_bbox_u, bbo, weightxff)

        loc_loss2_b = weight_l1_loss(loc_b, labelxff, weightxff)
        loc_loss2_o = weight_l1_loss(loc_o, labelxff, weightxff)
        loc_loss2_u = weight_l1_loss(loc_u, labelxff, weightxff)

        loc_loss1 = loc_loss1_b + loc_loss1_o + loc_loss1_u
        loc_loss2 = loc_loss2_b + loc_loss2_o + loc_loss2_u
        loc_loss3 = loc_loss3_b + loc_loss3_o + loc_loss3_u

        loc_loss_b = cfg.TRAIN.w1 * loc_loss1_b + cfg.TRAIN.w2 * loc_loss2_b + cfg.TRAIN.w3 * loc_loss3_b
        loc_loss_o = cfg.TRAIN.w1 * loc_loss1_o + cfg.TRAIN.w2 * loc_loss2_o + cfg.TRAIN.w3 * loc_loss3_o
        loc_loss_u = cfg.TRAIN.w1 * loc_loss1_u + cfg.TRAIN.w2 * loc_loss2_u + cfg.TRAIN.w3 * loc_loss3_u

        cls_loss_b = cfg.TRAIN.w4 * cls_loss2_b + cfg.TRAIN.w5 * cls_loss3_b
        cls_loss_o = cfg.TRAIN.w4 * cls_loss2_o + cfg.TRAIN.w5 * cls_loss3_o
        cls_loss_u = cfg.TRAIN.w4 * cls_loss2_u + cfg.TRAIN.w5 * cls_loss3_u

        basic_loss = cfg.TRAIN.LOC_WEIGHT * loc_loss_b + cfg.TRAIN.CLS_WEIGHT * cls_loss_b
        overall_loss = cfg.TRAIN.LOC_WEIGHT * loc_loss_o + cfg.TRAIN.CLS_WEIGHT * cls_loss_o
        update_loss = cfg.TRAIN.LOC_WEIGHT * loc_loss_u + cfg.TRAIN.CLS_WEIGHT * cls_loss_u

        outputs = {}
        outputs['total_loss'] = basic_loss + overall_loss + update_loss

        outputs['cls_loss'] = cls_loss_b + cls_loss_o + cls_loss_u
        outputs['loc_loss1'] = loc_loss1
        outputs['loc_loss2'] = loc_loss2
        outputs['loc_loss3'] = loc_loss3
        outputs['loc_loss'] = loc_loss_b + loc_loss_o + loc_loss_u
