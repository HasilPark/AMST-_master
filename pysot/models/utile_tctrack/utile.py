...
class TcT(nn.Module):
    ...
    def forward(self, x, z):
        ...
        px = x[2][:, :-1, :, :, :].permute(1, 0, 2, 3, 4)

        ppres = self.conv1_time(self.xcorr_depthwise(px[0], z[2]))

        for i in range(len(px)):
            res3 = self.conv2_time(self.xcorr_depthwise(px[i], z[2]))

            b, c, w, h = res3.size()
            memory_time = self.transformer_time_encoder((res3).view(b, c, -1).permute(2, 0, 1), \
                                                   (ppres).view(b, c, -1).permute(2, 0, 1))

            ppres = memory_time.permute(1, 2, 0).view(b, c, w, h)

        res3 = self.conv2_time(self.xcorr_depthwise(x_[2], z[2]))

        memory_time = self.transformer_time_encoder((res3).view(b, c, -1).permute(2, 0, 1), \
                                                    ppres.view(b, c, -1).permute(2, 0, 1))

        res1_ = self.conv1_spatio(self.xcorr_depthwise(x_[0], z[0]))
        res2_ = self.conv2_spatio(self.xcorr_depthwise(x_[1], z[1]))
        ...
        b, c, w, h = res3.size()
        memory_spatio = self.transformer_spatio_encoder((pos + res1_).view(b, c, -1).permute(2, 0, 1),
                                                        (pos + res2_).view(b, c, -1).permute(2, 0, 1))

        agg_memory = self.transformer_aggregation_encoder(memory_time, memory_spatio)

        res = self.share_decoder(agg_memory, memory_time, memory_spatio, res3.view(b, c, -1).permute(2, 0, 1))
