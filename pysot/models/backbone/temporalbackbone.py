...
class TemporalAlexNet(nn.Module):
    ...
    def __init__(self, width_mult=1):
        ...
        self.temporalconv1 = CondConv2d(configs[2], configs[3], kernel_size=3)

        self.b_f1 = nn.Sequential(nn.BatchNorm2d(configs[3]), nn.ReLU(inplace=True))

        self.temporalconv2 = CondConv2d(configs[3], configs[4], kernel_size=3)
        
        self.b_f2=  nn.Sequential(nn.BatchNorm2d(configs[4]), nn.ReLU(inplace=True))

        self.temporalconv3 =CondConv2d(configs[4], configs[5], kernel_size=3)
        
        self.b_f3= nn.BatchNorm2d(configs[5])
    ...
    def init(self, xset):
        ...
        xset = xset.unsqueeze(1)
        xset, feat3 = self.temporalconv3.initset(xset)
        xset = self.b_f3(xset)
        xset_3 = xset

        return [xset_1, xset_2, xset_3], feat1, feat2, feat3

    def eachtest(self, xset, feat1, feat2, feat3):
        ...
        xset = xset.unsqueeze(1)
        xset, feat3 = self.temporalconv3.conti(xset, feat3)
        xset = self.b_f3(xset)
        xset_3 = xset
    ...
    def forward(self, xset):
        ...
        xset = xset.view(B, L, xset.size(-3), xset.size(-2), xset.size(-1))
        xset = self.temporalconv3(xset)
        xset = self.b_f3(xset)
        xset_3 = xset

        return [xset_1, xset_2, xset_3]
