...
class TrkDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        update_image = cv2.imread(previous[-1][0])
        current_searchimg = cv2.imread(current[1][0])

        # template_link = current[0][0]
        # update_link = previous[-1][0]
        # search_link = current[1][0]
        rand = []
        for num in range(4):
        ...
        update_box = self._get_bbox(update_image, previous[-1][1])
        ...
        update_image, _ = self.template_aug(update_image,
                                            update_box,
                                            cfg.TRAIN.EXEMPLAR_SIZE,
                                            gray=gray)
        ...
        previous_searchimg = np.zeros(
            (current_searchimg.shape[0], current_searchimg.shape[1], current_searchimg.shape[2], cfg.TRAIN.videorange))
        ...
        update_image = update_image.transpose((2, 0, 1)).astype(np.float32)
